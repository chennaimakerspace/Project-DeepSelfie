	...
	def getTweets(tweetCriteria, receiveBuffer = None, bufferLength = 100):
		...
		while active:
			...
			for tweetHTML in tweets:
				...
				img_result = set(re.findall("https://pbs.twimg.com/media/.*jpg",tweetPQ.html()))
				geo = ''
				...
				if len(img_result)>0:
					for i in img_result:
						temp_string +=i+" "			
				tweet.many = temp_string
				if len(img_result) >0:
					results.append(tweet)
					resultsAux.append(tweet)
				
				if receiveBuffer and len(resultsAux) >= bufferLength and len(img_result)>0:
