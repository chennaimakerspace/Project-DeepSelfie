...
def main(argv):
 ...
 python Exporter.py --username 'DeepikaPFC' --since 2015-09-10 --until 2015-09-12 --maxtweets 1\n"""
	...
	try:
		...
		outputFile.write('username;date;retweets;favorites;text;geo;mentions;hashtags;id;permalink;imagelinks')
		
		print 'Searching...\n'
		
		def receiveBuffer(tweets):
			for t in tweets:
				outputFile.write(('\n%s;%s;%d;%d;"%s";%s;%s;%s;"%s";%s;%s' % (t.username, t.date.strftime("%Y-%m-%d %H:%M"), t.retweets, t.favorites, t.text, t.geo, t.mentions, t.hashtags, t.id, t.permalink, t.many)))
			outputFile.flush();
